...
```
  ...
  - 0で不連続になっているのは, こうした方がジョイスティックを触っていないときに安定してブレーキ状態になるため.
...
- Servo6221MG.hは昔の先輩が作ったライブラリを改造したもの.
...
```
- ハンド制御部
- 今回は空気圧を利用した電磁弁のOnOffでハンドを制御したので簡単な実装.
- Lチカの要領.

## 制御の反省点
- ハンド移動をグリッド制御にして, ワークの位置に来たら自動でいい感じに止まってくれるようにしたらもっと面白かったかもしれない.
- 回転と移動速度が遅い. DCモータのギア比をもっと小さくすべきだった.
- PIDのパラメータの決定に手間取りすぎた.  
- mbed側がマスクしすぎて8個あるTIMのうち4個しか使えなかった. ピンアサインの相談不足のために貴重なPWMのピンをただのDigitalOutに消費してしまった.
